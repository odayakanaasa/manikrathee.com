<?php $section='universalshanti'; $upper='dark'; $project='true'; ?>
<?php include('global/header.php'); ?>

<body class="interior shanti">
	<div class="upper shanti_upper">
		<?php include('global/nav.php'); ?>
		<div class="container">
			<div class="row">
				<div class="onecol"></div>
				<div class="fourcol">
				<div class="shanti_title">
					<img src="img/shanti_heading.png">
				</div>
				</div>
				<div class="sevencol last"></div>
			</div><!-- END row -->
		</div>
	</div>
	<div class="container body">
		<div class="row section_spacer">
			<div class="fourcol left-section">
				<div class="right_col_inner">
					<div class="headline">
						<h3 class="fit" class="glance">Project Details</h3>
					</div>
					<p class="glance_heading">Client<span>Universal Shanti</span></p>
					<p class="glance_heading">Project Type<span>Responsive Website &amp; CMS</span></p>
					<p class="glance_heading">Target Devices<span>Desktop / Mobile</span></p>
					<a class="button" href="http://www.universalshanti.com">View Project</a>
				</div>
			</div>
			<div class="onecol"></div>
			<div class="sixcol main-section">
				<div class="left_col_inner">
						<div class="article">
							<div class="headline">
								<h2 class="fit">Responsive CMS</h2>
							</div>
							<p>The Universal Shanti project was a redesign of a site I created in 2009. I kept the design almost identical but re-coded the entire thing onto my custom responsive framework. (Build on a combination of 1140gs, html5 boiler, fittext and more) The entire project needed to be built on wordpress so the client could continue to update the blog and pages herself.</p>
						</div>
						<div class="article">
							<div class="headline">
								<h2 class="fit">Non-Mobile First</h2>
							</div>
							<p>This was one of my first responsive projects so there are very obvious flaws that I use as a learning point. Loading time on mobile view is painfully slow - the header images are very large and the site was not designed with the mobile-first methodology. This causes excessive loading times on bandwidth sensitive devices. From a functional perspective, the site is just fine. It adapts to all viewports and displays content and dynamic buttons differently for the proper use-cases.</p>
						</div>
				</div>
			</div>
		</div><!-- END row -->
		<div class="row">
			<div class="twelvecol last">
				<div class="row">
					<div class="onecol"></div>
					<div class="tencol no_pad">
							<div id="slider" class="shanti_slides">
								<div class="flexslider">
								    <ul class="slides">
								    	<li>
								    		<img src="img/shanti_slides_1.jpg">
								    	</li>
								    	<li>
								    		<img src="img/shanti_slides_2.jpg">
								    	</li>
								    	<li>
								    		<img src="img/shanti_slides_3.jpg">
								    	</li>
								    	<li>
								    		<img src="img/shanti_slides_4.jpg">
								    	</li>
								    </ul>
							    </div>
							</div>

					</div>
					<div class="onecol last"></div>
				</div>
			</div>
		</div>
	</div><!-- END container -->




	<?php include('global/footer.php'); ?>
