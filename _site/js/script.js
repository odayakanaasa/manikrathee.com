...
if ("home" === $("body").attr("id")) {
    var instagramActive = !1, instagramFeed = new function() {
        this.endpoint = "instagram.me", this.render_html = function(a) {
            var b = "";
            for (i in a) {
                var c, d = a[i];
                c = "undefined" !== d.caption && "null" !== d.caption && d.caption ? d.caption.text : "Photo", 
                b += "<div class='photo' data='" + d.id + "'>", b += "<p>My latest instagram: <a id='instagram-link' href='" + d.link + "' title='View my latest Instagram Shot'>'" + c + "'", 
                b += "</a></p>", b += "</div>";
            }
            return b;
        }, this.render = function(a, b) {
            var c = document.getElementById(a), d = this.render_html(b);
            c.innerHTML = d;
        }, this.embed = function(a) {
            var b = ("http://" + this.endpoint + "/stylesheets/feed.css", "http://" + this.endpoint + "/feed.json?");
            b += "user=" + a.username, b += "&callback=instagramFeed.render", b += "&callback_container=" + a.container, 
            a.count && (b += "&count=" + a.count);
            var c = "<script type='text/javascript' src='" + b + "'></script>";
            document.write(c);
        };
    }();
    instagramFeed.embed({
...
imageSelectDropDownArrow = "selectDrop.gif", function(a) {
    a.fn.countTo = function(b) {
        ...
        return a(this).each(function() {
            function e() {
                h += d, g++, a(f).html(h.toFixed(b.decimals)), "function" == typeof b.onUpdate && b.onUpdate.call(f, h), 
                g >= c && (clearInterval(i), h = b.to, "function" == typeof b.onComplete && b.onComplete.call(f, h));
            }
    ...
    a.fn.tipTip = function(b) {
        ...
        return this.each(function() {
            function b() {
                d.enter.call(this), f.html(i), e.hide().removeAttr("class").css("margin", "0"), 
                g.removeAttr("style");
                ...
                v && 0 > o || "_right" == s && !w || "_left" == s && c < m + d.edgeOffset + 5 ? (s = "_right", 
                t = Math.round(n - 13) / 2, u = -12, q = Math.round(c + k + d.edgeOffset), r = Math.round(b + p)) : (w && 0 > o || "_left" == s && !v) && (s = "_left", 
                t = Math.round(n - 13) / 2, u = Math.round(m), q = Math.round(c - (m + d.edgeOffset + 5)), 
                r = Math.round(b + p));
                ...
                t = n, r = Math.round(b - (n + 5 + d.edgeOffset))) : (y | ("_top" == s && y) || "_bottom" == s && !x) && ("_top" == s || "_bottom" == s ? s = "_bottom" : s += "_bottom", 
                t = -12, r = Math.round(b + l + d.edgeOffset)), "_right_top" == s || "_left_top" == s ? r += 5 : ("_right_bottom" == s || "_left_bottom" == s) && (r -= 5), 
                ("_left_top" == s || "_left_bottom" == s) && (q += 5), g.css({
                    "margin-left": u + "px",
                    "margin-top": t + "px"
                }), e.css({
                    "margin-left": q + "px",
...
}), $(".flexslider").flexslider(), $(".tooltip").tipTip({
    maxWidth: "auto",
    edgeOffset: 10
});
